...
public class ProcessesFinal {
			...
			if(iniFlag == false){	
				...
				ProcessHandle.allProcesses().forEach(process -> {
					...
					if(!whitelist.contains(process.pid())){
						System.out.println("Destroyed - PID: "+ process + " Path Name: " + process.info().command());
	...
	private static void addWhitelist(ProcessHandle process) throws InterruptedException {
		...
		if(cmdFlag == false && (s.contains("C:\\Windows\\System32\\cmd.exe") || s.contains("terminal"))){
			whitelist.add(pid);
			cmdFlag = true;
			System.out.println(s + " heee1");
			process.destroy(); //this kills BOTH linux terminals?  they are connected processes apparently.
		}else if(cmdFlag2 == false && cmdFlag == true && (s.contains("C:\\Windows\\System32\\cmd.exe") || s.contains("terminal"))){
			badpid = process;
			whitelist.add(pid);
			cmdFlag2 = true;
			process.destroy();
		}else if(!s.contains("C:\\Windows\\System32\\cmd.exe") 
			|| !s.contains("terminal")){
			whitelist.add(pid);
